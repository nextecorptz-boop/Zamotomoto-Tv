...
function StatCard({ label, value, subtext, color = '#FFFFFF' }: { label: string; value: number | string; subtext?: string; color?: string }) {
  return (
    <div style={{ background: '#111111', border: '1px solid #2A2A2A', padding: '1.25rem 1.5rem' }}>
      ...
      {subtext && (
        <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.55rem', color: '#555555', marginTop: '0.2rem' }}>
          {subtext}
        </div>
      )}
    </div>
    ...
    <div data-testid="engagement-dashboard-client">
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
          ...
          <StatCard label="This Month" value={operator_stats.total_this_month} subtext="all submissions" />
        </div>

        {/* Progress bar */}
        <div style={{ background: '#111111', border: '1px solid #2A2A2A', padding: '1rem 1.25rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
            <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.62rem', color: '#888888', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Daily Target Progress</span>
            <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.7rem', color: progressPct >= 100 ? '#22C55E' : '#FFFFFF', letterSpacing: '0.05em' }}>
          ...
          {progressPct < 100 && (
            <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.58rem', color: '#555555', marginTop: '0.35rem' }}>
              {Math.max(0, operator_stats.daily_target - operator_stats.today_submitted)} submissions to reach daily target
            </div>
          )}
          {progressPct >= 100 && (
            <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.58rem', color: '#22C55E', marginTop: '0.35rem' }}>
      ...
      {isAdmin && (
        <div style={{ marginBottom: '2rem' }}>
          ...
          <div style={{
            background: pending_count > 0 ? 'rgba(245,158,11,0.08)' : '#111111',
            border: `1px solid ${pending_count > 0 ? '#F59E0B' : '#2A2A2A'}`,
            ...
            <div>
              <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.68rem', color: '#FFFFFF', letterSpacing: '0.05em' }}>
          ...
          <div style={{ background: '#111111', border: '1px solid #2A2A2A' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2rem 2fr 1fr 1fr 1fr', borderBottom: '1px solid #2A2A2A' }}>
              {['#', 'OPERATOR', 'SUBMITTED', 'APPROVED', 'MONTH'].map(col => (
                <div key={col} style={{ padding: '0.5rem 0.85rem', fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.55rem', color: '#555555', letterSpacing: '0.15em' }}>
                  {col}
                </div>
              ))}
            </div>
            {team_entries.map((entry, idx) => (
              <div
                ...
                style={{
                  display: 'grid', gridTemplateColumns: '2rem 2fr 1fr 1fr 1fr',
                  borderBottom: idx < team_entries.length - 1 ? '1px solid #1A1A1A' : 'none',
                  background: idx === 0 ? 'rgba(204,31,31,0.05)' : 'transparent',
                }}
              >
                <div style={{ padding: '0.65rem 0.85rem', fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.65rem', color: idx === 0 ? '#CC1F1F' : '#555555' }}>
                  {idx + 1}
                </div>
                <div style={{ padding: '0.65rem 0.85rem', fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.72rem', color: '#FFFFFF' }}>
                  {entry.operator_name ?? 'Unknown'}
                </div>
                <div style={{ padding: '0.65rem 0.85rem', fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.72rem', color: '#FFFFFF' }}>
                  {entry.today_count}
                </div>
                <div style={{ padding: '0.65rem 0.85rem', fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.72rem', color: '#22C55E' }}>
                  {entry.approved_count}
                </div>
                <div style={{ padding: '0.65rem 0.85rem', fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.72rem', color: '#888888' }}>
                  {entry.month_total}
                </div>
              </div>
            ))}
